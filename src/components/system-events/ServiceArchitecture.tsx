...
const SERVICES = [
    ...
    { id: 'Fraud Service', label: 'Fraud Engine', icon: ShieldCheck, pos: 'col-start-2 row-start-1 mt-4' },
    ...
    { id: 'Notification Service', label: 'Notifications', icon: Mail, pos: 'col-start-3 row-start-1 mt-4' },
];
...
export default function ServiceArchitecture( { events }: ServiceArchitectureProps ) {
    ...
    return (
        <div className={cn(
            "rounded-xl border shadow-sm p-5 h-full flex flex-col items-center justify-center relative overflow-hidden",
            ...
            <h3 className={cn( "text-sm font-semibold mb-8 self-start flex items-center gap-2", isDark ? "text-slate-200" : "text-slate-800" )}>
                <Network size={16} className={isDark ? "text-emerald-400" : "text-emerald-600"} />
                Service Architecture
            </h3>

            <div className="grid grid-cols-3 grid-rows-3 gap-6 w-full max-w-lg relative isolate">
                ...
                <svg className="absolute inset-0 w-full h-full -z-10 pointer-events-none stroke-current" style={{ color: isDark ? '#334155' : '#cbd5e1' }}>
                    ...
                    <line x1="16%" y1="50%" x2="50%" y2="50%" strokeWidth="2" strokeDasharray="4 4" className={activeServices.has( 'Payment Service' ) ? "opacity-100 animate-pulse" : "opacity-40"} />
                    <line x1="50%" y1="50%" x2="50%" y2="20%" strokeWidth="2" strokeDasharray="4 4" className={activeServices.has( 'Fraud Service' ) ? "opacity-100 animate-pulse" : "opacity-40"} />
                    <line x1="50%" y1="50%" x2="84%" y2="50%" strokeWidth="2" strokeDasharray="4 4" className={activeServices.has( 'Settlement Service' ) ? "opacity-100 animate-pulse" : "opacity-40"} />
                    <line x1="50%" y1="50%" x2="84%" y2="20%" strokeWidth="2" strokeDasharray="4 4" className={activeServices.has( 'Notification Service' ) ? "opacity-100 animate-pulse" : "opacity-40"} />
                </svg>
                ...
                {SERVICES.map( service => {
                    ...
                    return (
                        <div
                            key={service.id}
                            className={cn(
                                "flex flex-col items-center justify-center p-3 rounded-xl border relative transition-all duration-300",
                                service.pos,
